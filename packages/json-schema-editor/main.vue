<template>
  <div class="json-schema-editor">
      <a-row class="row" :gutter="10">
        <a-col :span="6" class="ant-col-name">
          <div :style="{marginLeft:`${20*deep}px`}" class="ant-col-name-c">
            ...
            <a-input size="small" :disabled="disabled || root" :default-value="pickKey" class="ant-col-name-input" @blur="onInputName" :key="pickValue"/>
          </div>
          <a-tooltip v-if="root">
            <template v-slot:title>{{ local['checked_all'] }}</template>
            <a-checkbox size="small" :disabled="!isObject && !isArray"  class="ant-col-name-required" @change="onRootCheck"/>
          </a-tooltip>
          <a-tooltip v-else>
        ...
        <a-col :span="4">
          <a-select size="small" v-model:value="pickValue.type" :disabled="disabledType" class="ant-col-type" @change="onChangeType" :getPopupContainer="
          triggerNode => {
        ...
        <a-col :span="4">
          <a-input size="small" v-model:value="pickValue.title" class="ant-col-title"/>
        </a-col>
        <a-col :span="4" v-if="(null != parent && parent.default_show) || pickValue.default_show ">
          <a-input size="small" v-model:value="pickValue.default" class="ant-col-title"/>
        </a-col>
      ...
      <template v-if="isArray">
        <json-schema-editor  :value="{items:pickValue.items}" :deep="deep+1" disabled isItem :root="false" :parent="pickValue" class="children" :lang="lang" :custom="custom"/>
      </template>
...
export default {
  ...
  methods: {
    ...
    onChangeType() {
      ...
      this.pickValue.default_show = this.parent.default_show
      if(this.isArray){
    ...
    addChild(){
      ...
      props[name] = {type: type, default_show: this.pickValue.default_show} //this.$set(props,name,{type: type})
    },
    parseCustomProps () {
...
.json-schema-editor .row .ant-col{
  display: flex;
  align-items: center;
}
